...
class SpatioTemporalResnetPointnet(nn.Module):
    ...
    def forward(self, x):
        ...
        spatial_c = self.spatial_fc_c(
            self.actvn(net))  #batch_size x n_steps x c_dim
        ...
        if self.pool_once:
            ...
            net = torch.cat([
                net,
                pooled.expand(net.size()),
                pooled_time.expand(net.size())
            ],
                            dim=3)
        ...
        spatiotemporal_c = self.fc_c(
            spatiotemporal_c)  #batch_size x n_steps x c_dim
...
class SpatioTemporalResnetPointnet2(nn.Module):
    ...
    def forward(self, x):
        ...
        spatiotemporal_c = torch.cat([spatial_c, temporal_c], dim=2)
        spatiotemporal_c = self.fc_c(
            self.actvn(spatiotemporal_c))  #batch_size x n_steps x c_dim
