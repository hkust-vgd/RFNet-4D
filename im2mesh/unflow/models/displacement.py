...
class DisplacementDecoder(nn.Module):
    ...
    def forward(self, p, cur_t, fuc_t, c):
        ...
        cur_t = torch.clamp(cur_t[:, :, None] * nsteps, 0, nsteps - 1).expand(
            batch_size, 1, c_dim).type(torch.LongTensor).to(p.device)
        fuc_t = torch.clamp(fuc_t[:, :, None] * nsteps, 0, nsteps - 1).expand(
            batch_size, 1, c_dim).type(torch.LongTensor).to(p.device)
