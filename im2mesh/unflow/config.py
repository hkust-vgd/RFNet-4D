...
from im2mesh.unflow import models, training, generation
...
def get_velocity_field(cfg, device, dim=3, c_dim=0):
    ...
    if velocity_field == 'concat':
        velocity_field = models.velocity_field_dict[velocity_field](
            out_dim=dim, c_dim=c_dim, **velocity_field_kwargs).to(device)
    else:
        velocity_field = None
...
def get_encoder(cfg, device, dataset=None, c_dim=0):
    ...
    if encoder == 'idx':
        if cfg['model']['learn_embedding']:
            encoder = nn.Sequential(nn.Embedding(len(dataset), 128),
                                    nn.Linear(128, c_dim)).to(device)
        else:
            encoder = nn.Embedding(len(dataset), c_dim).to(device)
    elif encoder is not None:
        encoder = encoder_dict[encoder](c_dim=c_dim,
...
def get_model(cfg, device=None, dataset=None, **kwargs):
    ''' Returns an UnFlow model instance.
    ...
    model = models.UnFlowNet(decoder=decoder,
                             encoder=encoder,
...
def get_trainer(model, optimizer, cfg, device, **kwargs):
    ''' Returns an UnFlow trainer instance.
    ...
    loss_corr_bw_only = cfg['model']['loss_corr_bw_only']
    ...
    trainer = training.Trainer(model,
                               ...
                               loss_corr_bw=loss_corr_bw,
                               loss_transform_forward=loss_transform_forward,
                               loss_corr_bw_only=loss_corr_bw_only)
    return trainer


def get_generator(model, cfg, device, **kwargs):
    ''' Returns an UnFlow generator instance.

    It provides methods to extract the final meshes from the UnFlow
    representation.
...
def get_transforms(cfg):
    ...
    transf_pt_back = data.SubsamplePointsSeq(n_pt,
...
def get_data_fields(mode, cfg):
    ...
    # Transformation
    transf_pt, transf_pt_back, transf_pt_val, transf_pcl, transf_pcl_val = get_transforms(
        cfg)
    ...
    if mode == 'train':
        if cfg['model']['loss_recon']:
            fields['points'] = pts_iou_field(p_folder,
                                             transform=transf_pt,
                                             seq_len=seq_len,
                                             fixed_time_step=0,
        ...
        fields['mesh'] = data.MeshSubseqField(mesh_folder,
                                              seq_len=seq_len,
